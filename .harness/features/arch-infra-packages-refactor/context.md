...
- `@chenrun/auth` (多租户认证与上下文核心)
- `@chenrun/authorization` (四层细粒度权限判定核心)
- `@chenrun/db-control` (多租户管控面与拓扑元数据事实源)
- `@chenrun/db-tenant` (多租户数据面物理库连接池与组织业务地基)
...
2. **内聚与分层 (Cohesion & Layering)**：
   - `@chenrun/db-control`: 拆分为 `contracts/`、`repositories/`、`prisma/`、`cli/`。
   - `@chenrun/db-tenant`: 拆分为 `pool/`、`migration/`、`topology/`、`seed/`。
   - `@chenrun/authorization`: 拆分为 `core/`、`scopes/`、`fields/`、`ability/`、`adapters/`。
   - `@chenrun/auth`: 拆分为 `server/`、`context/`、`client/`。
3. **模块自解释与设计资产沉淀**：
