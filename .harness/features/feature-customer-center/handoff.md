...
1. **客户中心业务切片与 UI 架构重构**:
   ...
   - 客户中心 4 张页面全量接入 `@chenrun/ui`（`BusinessTableWorkspace` + `DictionarySectionCard` + `Tabs` + `Button` + `Input` + `Badge`）；
   - 消除全包 `any` 逃逸，补全强类型模型与详尽中文 JSDoc 注释；
   - 遵循 SuperJSON 官方规范注册 `Decimal.js`，通过 `@chenrun/shared` 的 `toPlainData` 彻底消除 RSC 跨界 Decimal 报错；
2. **多租户数据迁移架构与命名重构**:
...
3. **全局工具底座与设计系统升级**:
   ...
   - 在 `@chenrun/shared` 引入 `radash`、`dayjs`、`superjson` 并优化集合与格式化工具；
4. **门禁与测试**:
