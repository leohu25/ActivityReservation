...
- [x] 搭建独立 Feature 包 `@base/feature-customer-center` 与 Prisma Schema 内聚模型（表及字段全中文注释）
...
- [x] 工业级数据迁移与命名规范统一重构：
  ...
  - [x] 抽取公共迁移对比与哈希计算工具至 `@base/shared`
...
- [x] 公共 UI 模块规范化与客户中心全量重构：
  ...
  - [x] 客户中心 4 张页面全量接入 `@base/ui`（`BusinessTableWorkspace` + `DictionarySectionCard` + `Tabs` + `Button` + `Input` + `Badge`）
  - [x] 彻底消除 `any` 逃逸，全面补齐 `CustomerListItem`、`StoreListItem`、`QuoteListItem` 强类型契约与完备中文注释
  - [x] 遵循 SuperJSON 官方规范注册 `Decimal.js`，通过 `@base/shared` 的 `toPlainData` 彻底根治 Next.js RSC 跨界 Decimal 报错
  - [x] 升级 `@base/shared` 全局底座，引入 `radash`、`dayjs`、`superjson` 并优化集合与格式化工具
