...
- `pnpm --filter @chenrun/db-migrate test`：18/18 PASS（新增事务级锁超时与云扩展表兼容测试）。
- `pnpm --filter @chenrun/feature-control-admin test`：3/3 PASS（覆盖超管环境变量回退与生产 Fail-Closed 鉴权测试）。
