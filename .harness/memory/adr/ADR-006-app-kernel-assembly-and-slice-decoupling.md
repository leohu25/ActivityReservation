...
- 导致 `tenant-admin` 在 `package.json` 中反向依赖了兄弟业务切片 `@base/feature-customer-center` 与 `@base/feature-procurement-center`。
...
2. **切片间横向耦合**：其它业务页面获取当前用户权限时必须从 `@base/feature-tenant-admin/server` 引入 `getTenantSubjectPermissions`，导致系统管理业务切片污染了所有业务域；
...
2. **支撑业务切片彻底解耦 (`packages/features/tenant-admin`)**：
   - 彻底移除 `tenant-admin` 对 `@base/feature-customer-center` 和 `@base/feature-procurement-center` 的工作区依赖；
...
- **消除循环依赖死锁**：避免了将全局注册表下沉到基础库（`@base/authorization`）引发的拓扑环；
