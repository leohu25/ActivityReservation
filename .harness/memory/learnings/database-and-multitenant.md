...
## 5. 多态业务附件与零中间表设计 (sys_attachment 范式)

- **痛点与反模式**：
  在需要支持多附件的业务单据中（如合同、送货单、发票），传统做法是为每个业务单独建关系中间表（如 `customer_attachment_rel`），导致数据库表爆炸、双重 JOIN 查询性能低下、且极易产生悬空孤儿数据。
- **解法与铁律**：
  1. **全系统复用单表 `sys_attachment`**：通过 `(module, targetId, fieldKey)` 三要素多态定位，业务主表干干净净、零附件字段、零中间表；
  2. **列表页防 N+1 查询**：
     分页查询（20 条/页）展示附件时，严禁循环查库。必须采用**“主表分页 + 单次 IN 批量查附件 + 内存 Map 聚合”**，全流程仅产生 2 条轻量 SQL；
  3. **详情页与两阶段落盘**：
