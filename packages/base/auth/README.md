...
通用 SaaS 的**多租户身份认证与可信会话上下文核心（Authentication & Tenant Context Kernel）**。
...
- **租户准入门禁断言 (`assertTenantAccessGate`)**：在进入业务逻辑前严格断言员工档案（`EmployeeProfile.status === "ACTIVE"`），任何 `SUSPENDED`（暂停）、`TERMINATED`（离职）或未激活人员均在毫秒级以 Fail-Closed 阻断。
- **纯粹化基础设施 (零 UI 耦合)**：本包彻底移除了前端 UI 组件（如模态框、切换按钮下沉至 `packages/features/tenant-admin`），剥离了对 `@base/ui` 的依赖，成为纯粹的**认证与上下文内核**。
...
```ts
import { assertTenantAccessGate } from "@base/auth";
...
```tsx
"use client";
import { authClient, signIn, signOut } from "@base/auth/client";
...
3. **零 UI 依赖与单向倒置**：`auth` 仅作为认证底座，零依赖 `@base/ui`，仅依赖 `@base/db-control`（中心管控库），绝对不反向依赖上层应用或 `@base/authorization`。
