...
test("CustomerFormModal [新增模式]: 全字段开放，渲染立即创建按钮，无需编码输入", () => {
	...
	// 新增模式下绝不展示客户编码录入框
	assert.doesNotMatch(html, /客户编码/, "主数据去编码后不应出现编码输入框");
});

test("CustomerFormModal [编辑模式]: 完整回填全量字段，渲染保存修改按钮", () => {
	const html = renderToString(
		<CustomerFormModal
			open={true}
			inline={true}
...
test("CustomerFormModal [数据清空防御]: 重新打开新建模式时初始数据保持默认纯净", () => {
	...
	assert.equal(DEFAULT_CUSTOMER_VALUES.name, "");
