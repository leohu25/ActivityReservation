...
export function QuoteFormModal({
	...
	const [values] = useState<{
		...
		scopeType: record?.storeId
			? "STORE"
			: record?.customerId
				? "CUSTOMER"
				: "REGION",
		customerId: record?.customerId || customers[0]?.id || "",
		storeId: record?.storeId || stores[0]?.id || "",
	...
	return (
		<FormModal
			...
			title={
				isEdit
					? `编辑草稿报价单 [${record?.quoteNo || record?.id}]`
					: "拟定新报价单"
			}
				...
				const payload = {
					customerId:
						...
						formValues.scopeType === "STORE"
							? (formValues.customerId as string)
							: null,
					storeId:
						formValues.scopeType === "STORE"
							? (formValues.storeId as string)
							: null,
					regionCode:
