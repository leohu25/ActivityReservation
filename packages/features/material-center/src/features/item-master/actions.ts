...
export const createItemMasterAction = defineServerAction(
  async (input: {
    ...
    assertMaterialAbility(ability, StandardAction.CREATE, ItemMasterSubject);

    const created = await client.itemMaster.create({
      data: {
        ...
        acceptanceStandard: input.acceptanceStandard || null,
...
export const updateItemMasterAction = defineServerAction(
  async (input: {
    ...
    const { client, ability, userId } = await getTenantMaterialContext();
    assertMaterialAbility(ability, StandardAction.UPDATE, ItemMasterSubject);
    ...
    const updated = await client.itemMaster.update({
      ...
      data: {
        ...
        ...(input.itemCategory ? { itemCategory: input.itemCategory } : {}),
        ...(input.categoryId ? { categoryId: input.categoryId } : {}),
        ...(input.varietyId === undefined
          ? {}
          : { varietyId: input.varietyId }),
        ...(input.supplyMode ? { supplyMode: input.supplyMode } : {}),
...
export const deleteItemMasterAction = defineServerAction(
  async (input: { id: string }) => {
    const { client, ability, userId } = await getTenantMaterialContext();
    assertMaterialAbility(ability, StandardAction.DELETE, ItemMasterSubject);

    const deleted = await client.itemMaster.update({
      where: { id: input.id },
      data: {
        isDeleted: true,
        deletedAt: new Date(),
