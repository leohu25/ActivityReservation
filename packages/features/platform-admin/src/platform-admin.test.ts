...
test("平台超管鉴权判定与断言守卫", () => {
  ...
  assert.equal(isPlatformAdminEmail("admin@chenrun.com"), true);
  assert.equal(isPlatformAdminEmail("ADMIN@CHENRUN.COM"), true); // 大小写不敏感
  assert.equal(isPlatformAdminEmail("user@example.com"), false);
  assert.equal(isPlatformAdminEmail(null), false);
  assert.equal(isPlatformAdminEmail(""), false);

  // 2. checkIsPlatformAdmin
  assert.equal(checkIsPlatformAdmin({ email: "admin@chenrun.com" }), true);
  assert.equal(checkIsPlatformAdmin({ email: "normal@tenant.com" }), false);
  assert.equal(checkIsPlatformAdmin(null), false);

  // 3. assertPlatformAdmin
  assert.doesNotThrow(() => {
    assertPlatformAdmin({ email: "admin@chenrun.com" });
  });

  assert.throws(
    () => {
      assertPlatformAdmin({ email: "attacker@malicious.com" });
    },
    {
      name: "Error",
      message: /需要平台超级管理员权限/,
...
test("PlatformAdminService 租户开通逻辑与状态管控", async () => {
  ...
  const service = new PlatformAdminService(fakePrisma);
