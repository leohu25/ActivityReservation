 ...
 * 平台总控服务的依赖装配参数
 */
export interface PlatformAdminServiceOptions {
...
/**
 * 平台运营商总控核心服务 (Platform Admin Service)
 * 负责平台超管大盘统计、租户全生命周期管理、物理独立库自动开通 (Provisioning) 与状态管控。
 */
export class PlatformAdminService {
  ...
  /**
   * 工厂方法：基于显式依赖构建 PlatformAdminService
   */
  static create(options: PlatformAdminServiceOptions): PlatformAdminService {
    if (options.provisioner) {
      return new PlatformAdminService(options.prisma, options.provisioner);
    }
    ...
    return new PlatformAdminService(options.prisma, provisioner);
  }
   ...
   */
  async listTenants(): Promise<PlatformTenantItem[]> {
    const orgs = await this.prisma.organization.findMany({
      include: {
        tenantDatabase: true,
        members: {
  ...
  async provisionTenant(
    ...
    // 1. 平台超管权限断言
    assertPlatformAdmin(operatorUser);
