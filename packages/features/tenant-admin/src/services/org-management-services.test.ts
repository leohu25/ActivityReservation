...
test("DepartmentService 部门树形层级加载、防环调换与 Fail-Closed 删除防护", async () => {
  ...
  const mockTenantPrisma = {
    ...
    employeeProfile: {
      async groupBy({ by, where }: { by: string[]; where: { status: string; departmentId: { not: null } } }) {
...
test("PositionService 岗位字典 CRUD、在职人数统计与删除保护", async () => {
  ...
  const mockTenantPrisma = {
    ...
    employeeProfile: {
      async groupBy({ by, where }: { by: string[]; where: { status: string; positionId: { not: null } } }) {
...
test("EmployeeManagementService 直接录入建号、调岗调部门与版本号递增", async () => {
  ...
  const mockControlPrisma = {
    ...
    member: {
      async findUnique({
        ...
        where: { id?: string; organizationId_userId?: { organizationId: string; userId: string } };
      }) {
      ...
      async findMany({ where }: { where: { organizationId: string; id?: { in: string[] } } }) {
        return members
          .filter((m) => {
  ...
  const mockTenantPrisma = {
    ...
    employeeProfile: {
      async findUnique({ where }: { where: { id?: string; employeeNo?: string } }) {
        if (where.id) return profiles.find((p) => p.id === where.id) ?? null;
        if (where.employeeNo)
          return profiles.find((p) => p.employeeNo === where.employeeNo) ?? null;
        return null;
      },
          ...
          .filter((p) => {
            if (where?.departmentId) {
              if (where.departmentId.in) {
                if (!where.departmentId.in.includes(p.departmentId)) return false;
