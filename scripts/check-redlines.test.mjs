...
const scriptSource = await readFile(
  path.join(repositoryRoot, "scripts/check-redlines.mjs"),
  "utf8",
);
...
async function runFixture(files) {
  ...
  try {
    ...
    await writeFile(path.join(root, "scripts/check-redlines.mjs"), scriptSource);
    for (const [relativePath, content] of Object.entries(files)) {
      const fullPath = path.join(root, relativePath);
      await mkdir(path.dirname(fullPath), { recursive: true });
      await writeFile(fullPath, content);
    }
    return spawnSync(process.execPath, ["scripts/check-redlines.mjs"], {
      cwd: root,
      encoding: "utf8",
    });
  } finally {
...
test("redline rejects Horizontal Platform reverse dependency", async () => {
  const result = await runFixture({
    ...
    "packages/shared/src/index.ts":
      'import { value } from "@chenrun/feature-a";\nexport { value };\n',
    "packages/features/a/package.json": JSON.stringify({
      name: "@chenrun/feature-a",
    }),
    "packages/features/a/src/index.ts": "export const value = 1;\n",
  });

  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Horizontal Shared \/ Platform Module/);
});

test("redline rejects workspace package src path penetration", async () => {
  const result = await runFixture({
    "apps/tenant/package.json": JSON.stringify({
      name: "tenant",
      dependencies: { "@chenrun/shared": "workspace:*" },
    }),
    "apps/tenant/src/page.ts":
      'import { value } from "@chenrun/shared/src/internal";\nexport { value };\n',
    "packages/shared/package.json": JSON.stringify({ name: "@chenrun/shared" }),
    "packages/shared/src/internal.ts": "export const value = 1;\n",
  });
