...
export function extractScopeFromPath(filePath) {
	if (!filePath) return null;
	const match = /^tooling\/db-migrate\/(?:baselines|migrations)\/([^/]+)/.exec(
		normalizeGitPath(filePath),
	);
	return match ? match[1] : null;
}

export function findBaselineResetScopes(changes) {
	const scopes = new Set();
	for (const change of changes) {
		if (change.status === "A" || change.status === "R") {
			const match =
				/^tooling\/db-migrate\/baselines\/([^/]+)\/[^/]+\/baseline\.sql$/.exec(
					change.path,
				);
			if (match) {
				scopes.add(match[1]);
...
export function findImmutableMigrationViolations(changes) {
	const baselineResetScopes = findBaselineResetScopes(changes);

	return changes.filter((change) => {
		if (change.status === "A" || change.status === "C") return false;

		const isPathProtected = isProtectedMigrationArtifact(change.path);
		const isOldPathProtected =
			change.oldPath !== null && isProtectedMigrationArtifact(change.oldPath);

		if (!isPathProtected && !isOldPathProtected) {
			return false;
		}

		// 若当前 Scope 正在进行基线更新重置（生成了新 Baseline），允许删除旧基线/旧迁移历史或将旧基线文件重命名/合并
		const scope =
			extractScopeFromPath(change.path) || extractScopeFromPath(change.oldPath);
		if (scope && baselineResetScopes.has(scope)) {
	...
	if (violations.length > 0) {
		...
		process.stderr.write(
			"\n迁移历史必须 Append-Only：请还原上述文件，并通过新增迁移修复数据库结构。\n" +
				"若确需重置基线并合并历史，需确保包含新生成的 baseline.sql，或通过 ALLOW_MIGRATION_BASELINE_RESET=1 受控放行。\n",
		);
		process.exit(1);
	}

	process.stdout.write(
		"\x1b[32m• 迁移历史不可变门禁: 已提交迁移与基线保持合规 (Append-Only / 基线已自愈合并)\x1b[0m\n",
	);
}
